...
impl dyn BlockCipher {
    fn encrypt_block_cbc(&self, prev_block: &[u8], next_block: &mut [u8]) {
        for i in 0..next_block.len() {
            next_block[i] ^= prev_block[i];
        }

        self.encrypt_block(next_block);
    }

    pub fn cbc_encrypt(&self, iv: &Vec<u8>, plaintext: &mut Vec<u8>) -> Result<(), String> {
        ...
        let len: u128 = plaintext.len() as u128;
        ...
        plaintext.append(&mut ((zeros + (128 >> 3) + 1) as u128).to_le_bytes().to_vec());
        let new_len = plaintext.len();
        let mut prev_block: Vec<u8> = iv.clone();
        for i in 0..new_len / blocksize {
            self.encrypt_block_cbc(
                &prev_block[..],
                &mut plaintext[(i * blocksize)..((i + 1) * blocksize)],
            );
    ...
    fn decrypt_block_cbc(&self, prev_block: &[u8], next_block: &mut [u8]) {
        self.decrypt_block(next_block);
        for i in 0..next_block.len() {
            next_block[i] ^= prev_block[i];
        }
    }

    pub fn cbc_decrypt(&self, iv: &Vec<u8>, ciphertext: &mut Vec<u8>) -> Result<(), String> {
        if iv.len() != self.block_size() {
            return Err(format!(
                "input vector is wrong length, expected {}, got {}",
                self.block_size(),
        ...
        for i in 0..(ciphertext.len() / bs) {
            ...
            self.decrypt_block_cbc(&pb[..], &mut ciphertext[(i * bs)..((i + 1) * bs)]);
        }
